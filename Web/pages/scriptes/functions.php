...
// ============================================================================
// MODERN JAVASCRIPT INCLUSION FUNCTIONS (RECOMMENDED APPROACH)
// ============================================================================

/**
 * Include JavaScript files with proper configuration
 * This replaces the old string-generation approach with clean file inclusion
 */
function includeJavaScriptAssets($scripts = [], $config = []) {
    $output = '';
    
    // Determine the correct path based on where this function is being called from
    // If called from a page in /pages/, use ../assets/js/
    // If called from elsewhere, use assets/js/
    $basePath = (strpos($_SERVER['SCRIPT_NAME'], '/pages/') !== false) ? '../assets/js/' : 'assets/js/';
    
    // Include utilities first (always needed)
    $output .= '<script src="' . $basePath . 'utilities.js?v=' . time() . '"></script>' . "\n";
    
    // Include specific page scripts
    foreach ($scripts as $script) {
        $scriptPath = $basePath . "{$script}.js";
        $output .= '<script src="' . $scriptPath . '?v=' . time() . '"></script>' . "\n";
    }
    
    // Add configuration if provided
    if (!empty($config)) {
        $output .= '<script>' . "\n";
        $output .= 'window.' . ($config['namespace'] ?? 'PageConfig') . ' = ' . json_encode($config['data'] ?? $config) . ';' . "\n";
        $output .= '</script>' . "\n";
    }
...
/**
 * Include Beings page JavaScript with configuration
 */
function includeBeingsPageAssets($apiEndpoint = './scriptes/Beings_admin_interface.php', $isAdmin = false) {
    $config = [
    ...
    // Include utilities, beings functionality, and tab manager (needed for admin interface)
    return includeJavaScriptAssets(['beings'], $config) . 
           '<script src="' . $basePath . 'tab-manager.js?v=' . time() . '"></script>' . "\n" .
           '<script src="' . $basePath . 'entity-manager.js?v=' . time() . '"></script>' . "\n";
}

/**
 * Legacy compatibility function - outputs Beings JavaScript
 * Now uses clean file inclusion instead of generated strings
 */
function outputBeingsPageFunctions($apiEndpoint = './scriptes/Beings_admin_interface.php', $includeAdminFunctions = true) {
    echo includeBeingsPageAssets($apiEndpoint, $includeAdminFunctions);
}

/**
 * Include tab manager JavaScript assets
 * Modern replacement for generateTabSwitchingFunctions()
 */
function includeTabManagerAssets() {
    echo '<script src="assets/js/tab-manager.js" defer></script>' . PHP_EOL;
}

/**
 * Include character manager JavaScript assets
 * Modern replacement for generateCharacterPageFunctions()
 */
function includeCharacterManagerAssets() {
    echo '<script src="assets/js/utilities.js" defer></script>' . PHP_EOL;
    echo '<script src="assets/js/character-manager.js" defer></script>' . PHP_EOL;
    echo '<script src="assets/js/entity-manager.js" defer></script>' . PHP_EOL;
}

/**
 * Include all JavaScript assets for full admin functionality
 */
function includeAllJavaScriptAssets() {
    echo '<script src="assets/js/utilities.js" defer></script>' . PHP_EOL;
    echo '<script src="assets/js/tab-manager.js" defer></script>' . PHP_EOL;
    echo '<script src="assets/js/beings.js" defer></script>' . PHP_EOL;
    echo '<script src="assets/js/character-manager.js" defer></script>' . PHP_EOL;
    echo '<script src="assets/js/entity-manager.js" defer></script>' . PHP_EOL;
}

// ============================================================================
// LEGACY JAVASCRIPT GENERATION FUNCTIONS (DEPRECATED - KEPT FOR COMPATIBILITY)
// ============================================================================

/**
 * @deprecated Use includeJavaScriptAssets() and assets/js/utilities.js instead
...
function generateSharedJavaScriptUtilities() {
    error_log("Warning: generateSharedJavaScriptUtilities() is deprecated. Use includeJavaScriptAssets() and assets/js/utilities.js instead.");
    return "/* Shared utility functions moved to assets/js/utilities.js - use includeJavaScriptAssets() instead */";
}

/**
 * @deprecated Use assets/js/entity-manager.js instead
...
function generateEntityDeleteFunctions($entityType, $apiEndpoint = null, $dynamicUpdate = true) {
    error_log("Warning: generateEntityDeleteFunctions() is deprecated. Use assets/js/entity-manager.js instead.");
    return "/* Entity delete functions moved to assets/js/entity-manager.js */";
}
...
/**
 * @deprecated Use assets/js/beings.js instead
...
function generateBeingsPageFunctions($apiEndpoint = './scriptes/Beings_admin_interface.php') {
    error_log("Warning: generateBeingsPageFunctions() is deprecated. Use assets/js/beings.js instead.");
    return "/* Beings page functions moved to assets/js/beings.js */";
}

/**
 * @deprecated Use includeTabManagerAssets() instead
 * Generate shared tab functionality for admin interfaces
 * @return string JavaScript code for tab switching
 */
function generateTabSwitchingFunctions() {
    error_log("Warning: generateTabSwitchingFunctions() is deprecated. Use includeTabManagerAssets() instead.");
    return "/* Tab switching functions moved to assets/js/tab-manager.js - use includeTabManagerAssets() instead */";
}

/**
 * @deprecated Use includeCharacterManagerAssets() instead
...
function generateCharacterPageFunctions($apiEndpoint = './scriptes/Character_admin_interface.php') {
    error_log("Warning: generateCharacterPageFunctions() is deprecated. Use includeCharacterManagerAssets() instead.");
    return "/* Character management functions moved to assets/js/character-manager.js - use includeCharacterManagerAssets() instead */";
}
...
// New implementations should use separate .js files and includeJavaScriptAssets()

// Note: JavaScript functions were moved to separate .js files for better maintainability
// Use includeBeingsPageAssets() or includeJavaScriptAssets() instead
?>
