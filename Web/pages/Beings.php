    ...
    <div class="beings-grid">
        ...
        try {
            ...
            while ($species = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                <div class="species-card" data-species="<?php echo htmlspecialchars($species['specie_name']); ?>">
                    <!-- Species Header -->
                    <div class="species-header" onclick="toggleSpeciesRaces('<?php echo $species['id_specie']; ?>')">
                        ...
                        <div class="species-info">
                            ...
                            <p class="species-race-count"><?php echo count($races); ?> race(s)</p>
                    ...
                    <!-- Races Section -->
                    <div class="races-section" id="races-<?php echo $species['id_specie']; ?>" style="display: none;">
                        <?php if (empty($races)): ?>
                            <div class="no-races">
                        ...
                        <?php else: ?>
                            <div class="races-grid">
                                ...
                                ?>
                                <div class="race-card" onclick="viewRaceDetails(<?php echo $species['id_specie']; ?>, <?php echo $race['id_race']; ?>)">
...
function toggleSpeciesRaces(speciesId) {
    ...
    // Hide all other races sections first
    document.querySelectorAll('.races-section').forEach(section => {
        if (section.id !== 'races-' + speciesId) {
            section.style.display = 'none';
            section.closest('.species-card').classList.remove('expanded');
        }
    });
    
    if (racesSection.style.display === 'none' || racesSection.style.display === '') {
...
function openAdminModal() {
    ...
    fetch('./Beings_admin_interface.php')
...
function loadEditSpeciesForm(speciesId) {
    fetch(`./Beings_admin_interface.php?action=edit_species&id=${speciesId}`)
        .then(response => response.text())
        .then(html => {
            document.getElementById('adminModalContent').innerHTML = html;
        });
...
function loadAddRaceForm(speciesId) {
    fetch(`./Beings_admin_interface.php?action=add_race&species_id=${speciesId}`)
        .then(response => response.text())
        .then(html => {
            document.getElementById('adminModalContent').innerHTML = html;
        });
